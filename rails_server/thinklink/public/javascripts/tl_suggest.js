function tl_suggest(parentElem,attached, url, textarg) {
	...
	this.setDefaultText = function(text) {
		...
		this.textBox.addEventListener('focus',function(){
			if (that.textBox.value == that.defaultText) { that.textBox.value="";}
		},false);
	}
	...
	this.runQuery = function(q) {
		if (q==null) { q=this.textBox.value; }
		...
		doAJAX(scriptID,this.queryURL+args,function(result){	
	...
	this.showResults = function(result) {
		...
		for (var item=0; item<this.resultsList.length; item++) {
			...
			itemResult.addEventListener('mouseover',function() {this.className="suggest_item_over"; },false); 
			itemResult.addEventListener('mouseout',function() { this.className = "suggest_item"; },false); 
		...
		// don't do anything if the div is hidden
		  ...
		  if ((key == KEYENTER) || (key == KEYTAB)) {
		   	if (selDiv) {
				...
				if (classptr.selectCallback !=null) { classptr.selectCallback(selDiv); }
