...
def load_claims(filename): return [line.strip().lower() for line in file(nt.local_filename(__file__,filename)) if is_good(line)]
def load_claims_raw(filename): return [line.strip().lower() for line in file(filename) if is_good(line)]

def load_messy_claims(filename): return [line.strip() for line in file(nt.local_filename(__file__,filename)) if not is_good(line)]

good_gold = load_claims("good_claims_gold.txt")
bad_gold = load_claims("bad_claims_gold.txt")
all_claims = load_claims("/home/rob/git/thinklink/output/wicow_indata/shuffled_claims.claims")
