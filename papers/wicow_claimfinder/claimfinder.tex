...
{\bf either put here the other related work on this, or just put this reference in the related work section-- especially because there much more important work beyond hearst92, so no need to give it much emphasis in the intro}
...
We search the web for patterns that indicate that a claim is disputed, such as ``false claim that $X$'' or or ``it is not true that $X$''. Figure~\ref{templates} shows a subset of the patterns that we search for. For each pattern, we show the number of times that Yahoo estimates that the pattern appears on the web. 
...
{\bf it would be quite important to give a sense on the accuracy --precision - of the claims extracted. you can calculated this from a subset of returned candidate claims: out of, say, 100, how many were actually claims?}

{\bf is it really important the discussion on the date and search technique? i think it deviates from the main points and is kind of long}
