...
Manual inspection suggests that the classifier is fairly good at filtering out strings that are not claims (case 1) but does poorly at working out whether a claim is unambigous. This is not surprising. We anticipate that a more sophisticated technique with real world knowledge would be needed in order to determine whether a claim was unambigous.
...
We downloaded all text strings that matched each of our 54 patterns for all dates from October 1st 2009 to January 19th 2010, and for January 10th of every year from 2000 to 2010. This gave us a total of 1.8 million text strings, of which 1.1 million were identified by our classifier as being unambigous disputable claims. Of those 1.1 million text strings, most are repeating claims made elsewhere in the data set.
...
MemeTracker~\cite{Backstrom2009} produced vaguely similar graphs, showing when particular memes were widely discussed on the web. They tracked highly repeated phrases, rather than nouns used in disputed claims. 
...
For each noun, we analysed the claims about that noun, and produced clusters that represent claims about that noun that seem to be particularly hotly disputed. Rather than clustering claims directly, we instead cluster together words that frequently co-occur. We used an agglomerative clustering algorithm, repeatedly merging the two clusters of words that had the most claims in common.
